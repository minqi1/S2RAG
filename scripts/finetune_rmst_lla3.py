...
8, 11, 13, 16, 24, 25, 36
  ...
  - Description: main part of the code for llama2 training
-----------------------------------------------------------------
  
Modifications are from:

Contributors:
- Minqi Xiang (mx716@ic.ac.uk)

Private Source: https://github.com/minqi1/S2RAG
Description: modification of the original code to fit the llama3 training
...
def encode_with_prompt_completion_format(example, tokenizer, max_seq_length, context_markups=None):
    ...
    source_text = example['instruction']
    ...
    labels[ :source_len-1] = -100 # mask out the prompt tokens and retain only the completion tokens
...
def main():
    ...
    if isinstance(tokenizer, LlamaTokenizer) or isinstance(tokenizer, LlamaTokenizerFast) or isinstance(tokenizer, PreTrainedTokenizerFast): # for llama3
        special_token_dict = {}
        ...
        elif 'Llama-3-8B' in args.model_name_or_path:
            special_token_dict["unk_token"] = "<unk>"
            special_token_dict["pad_token"] = "<pad>"
        else:
            raise ValueError("we only train Llama-2-7b or Llama-3-8b models.")

        num_added_tokens = tokenizer.add_special_tokens(special_token_dict)
        
        context_markups = None
        
        assert num_added_tokens in [0, 1, 2], "LlamaTokenizer should only add 2 special token the pad_token, , or no tokens if pad token present."
        print('NOT USING SPECIAL TOKENS. num_added_tokens added to the vocab:', num_added_tokens)
    ...
    with accelerator.main_process_first():
        lm_datasets = raw_datasets.map(
            ...
            remove_columns=[name for name in raw_datasets["train"].column_names if name not in ["input_ids", "labels", "attention_mask"]], # remove other columns
