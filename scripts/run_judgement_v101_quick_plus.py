...
parser.add_argument("--tau", type=float, default=0.45, help="Threshold for proceeding") # set different val for different model with different confidence (llama2: 0.4, llama3: 0.2, selfrag: 0.2)
...
if args.use_trained:
    # base_dir = "../weights_minqi/generator_weights/llama3_8B_allft_left"
    base_dir = "../weights_minqi/generator_weights/llama3_8B_allft_left_256_3"
...
if args.base is None:
    ...
    elif 'lama2' in args.base and '13' in args.base:
        args.base = 'Llama-2-13b'
...
trained_idx = []
if args.test:
    #idx_path = f'/apdcephfs_qy3/share_4983883/ping_test/rag/minqi_dev/self-rag-main/data_training/train_data_{task}.jsonl'
    idx_path = f'/apdcephfs_qy3/share_4983883/ping_test/rag/minqi_dev/self-rag-main/data_training/train_data_fusion.jsonl'
...
for i, item in enumerate(tqdm(json_file, desc="Processing items")):
    ...
    while True and cur_pos <= len(prefix) - 2: 
        ...
        if (score_record.count(max_score) == 1 and cur_score_diff > args.tau) or both_good: # only best answer is noticeable better, OR first two are equally good
            ...
            print('='*80)
            stage2_cnt += 1 # count of stage 2: confidence judgment
            # record the error records for further analysis ONLY
            # error_rec.append({'stage': '2', 'task': task, 'question': question[-1], 'gold': golds[-1], 'best_pred': best_pred[-1], 'best': best[-1], 'internal_score': scores[-1], 'current_metric': performance[-1], 'possible_correct_pred': possible_correct_pred})                
...
if not args.use_trained:
    if args.test:
        with open(file_path.replace('.json', '-final_rec_test_plus.json'), 'w') as f:
            json.dump(final_rec, f)
    else:
        with open(file_path.replace('.json', '-final_rec_plus.json'), 'w') as f:
            json.dump(final_rec, f)
else:
    if args.test:
        if args.base == model_name:
            with open(file_path.replace('.json', f'-final_rec_test_trained_3_{args.tau}_plus.json'), 'w') as f:
                json.dump(final_rec, f)
        else:
            with open(file_path.replace('.json', f'-final_rec_test_trained_3_{args.tau}_{args.base}_plus.json'), 'w') as f:
                json.dump(final_rec, f)
    else:
        with open(file_path.replace('.json', f'-final_rec_trained_3_{args.tau}_plus.json'), 'w') as f:
            json.dump(final_rec, f)
